...
describe('getReportData', () => {
  ...
  test('shows the fixable summary when fixable counts are present', () => {
    ...
    expect(consoleMessage).toContain('2 errors and 1 warning potentially fixable with the `--fix` option.');
  });
  ...
  test('omits the fixable summary when fixable counts are zero', () => {
    ...
    expect(consoleMessage).not.toContain('potentially fixable with the `--fix` option.');
  });
  ...
  test('propagates real fixable counts through aggregation', () => {
    ...
    expect(consoleMessage).toContain('4 errors and 4 warnings potentially fixable with the `--fix` option.');
  });

  test('drops files with no problems from the report', () => {
    const { errorCount, warningCount, consoleMessage } = getReportData([
      makeItem({ lintResult: [], fixableErrorCount: 0, fixableWarningCount: 0 }),
    ]);
